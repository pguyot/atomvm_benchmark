...
%% benchmark, and is gracefully skipped when it relies on a function the running
%% machine does not implement (some micros use functions such as
%% `erts_debug:flat_size/1' or `ets' operations that AtomVM may not provide).
%% The `port_io' micro of the original benchmark is omitted as it requires an
%% external port program.

-module(estone_test).

%% erlang:now/0 is only used as a fallback on machines that do not implement
%% erlang:monotonic_time/0, see monotonic_time/0.
...
gserv(Name, Mod, State, Debug) ->
    receive
        {From, Ref, {call, Req}} when Debug == [] ->
            try apply(Mod, handle_call, [From, State, Req]) of
                {reply, Reply, State2} ->
                    From ! {Name, Ref, Reply},
